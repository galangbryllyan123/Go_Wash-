...
(($, Themify, win, topWindow, doc,und) => {
    ...
    tb_app = {
        ...
        jsModuleLoaded(){
            if(jsModulePrms===null){
				...
				baseUrl.search = '';
				baseUrl.pathname = baseUrl.pathname.replace(/themify-builder-app(\.min)?\.js/, '');
				const moduleUrl = baseUrl.toString() + 'modules/';
                    if(!Themify.isTouch){
                        jsModules.unshift('right-click');
                    }
                if(!Themify.builder_url){
                    Themify.builder_url=baseUrl.toString().replace('js/editor/','');
                }
    ...
    api.Helper={
        correctBuilderData(rows){
            ...
            for(let i=rows.length-1;i>-1;--i){
                if(rows[i]){
                    ...
                    if(rows[i].styling!==und && (!rows[i].styling || Array.isArray(rows[i].styling))){
                        rows[i].styling={};
                    }
                    if(cols){
                        ...
                        for(let j=cols.length-1;j>-1;--j){
                            if(cols[j]){
                                if(cols[j].styling!==und && (!cols[j].styling || Array.isArray(cols[j].styling))){
                                    cols[j].styling={};
                                }
                                if(cols[j].modules){
        ...
        isImageUrl(link) {
            ...
            return ['jpg', 'jpeg', 'tiff', 'png', 'gif', 'bmp', 'svg','webp','apng'].includes(parts[parts.length - 1]);
        },
        loadJsZip(){
          return Themify.loadJs('https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js',!!win.JSZip,false);
        },
        toRGBA(v){
            ...
            for (let i in items) {
                ...
                if (Array.isArray(items[i])) {
                    ...
                    if ((i === 'hide_anchor' && !items[i]) || (items[i] === 'show' && i.indexOf('visibility_') === 0) || (i === 'unstick_when_condition' && items[i] === 'hits') || (i === 'unstick_when_pos' && items[i] === 'this') || (i === 'unstick_when_element' && items[i] === 'builder_end') || ((i === 'stick_at_pos_val_unit' || i === 'unstick_when_pos_val_unit') && items[i] === 'px')) {
                        ...
                        let opt = [];
                        if (i.indexOf('checkbox_') === 0 && i.includes('_apply_all', 6)) {
                            if (!items[i]) {
                                opt.push(i);
                            } else {
                                res[i] = items[i];
                        ...
                        } else if (i.includes('gradient', 3) ) {
                            if (items[i] == '180' || items[i] === 'linear' || items[i] === $.ThemifyGradient.default || (items[i] === false && i.includes('-circle-radial', 3) )) {
                                opt.push(i);
                            }
                        } else if ((items[i] === 'px' && i.includes('_unit', 2)  && !i.includes('frame_')) || (items[i] === '%' && i.includes('_unit', 2) && i.includes('frame_') ) || (i === 'background_zoom' && items[i] === '') || (items[i] === 'none' && i.includes('frame_layout') ) || items[i] === 'solid' || (items[i] === false && (i.includes('_user_role', 3)  || i.includes('_appearance', 3) )) || ((!items[i] || items[i] === 'false') && (i === 'margin-top_opp_top' || i === 'm_t_h_opp_top' || i.indexOf('padding_opp_') === 0 || i.indexOf('margin_opp_') === 0))) {
                            opt.push(i);
                        }
                        if (opt.length > 0) {
    ...
    api.Base=class  {
            ...
            return new Promise(async(resolve,reject)=>{
                ...
                if (is_style === true) {
                    const stOptions = ThemifyStyles.getStyleOptions(component),
                        ...
                        res = this.getData(),
                        checkIsStyle = i=> {
                            if (i.includes('breakpoint_')  || i.includes('_apply_all') ) {
                                return true;
                            }
        ...
        options(input,type) {
            ...
            if(api.mode!=='visual' && event==='keyup'){
    ...
    api.Row =class extends api.Base {
        ...
        grid(target) {
            ...
            const count = grid ? (grid.includes('_')  ? grid.split('_').length : parseInt(grid)) : '',
        ...
        gridMenu(el) {
            ...
            let items = tpl.querySelector('.grid_list').children,
                {gutter,align,size,auto_h}=this.getSizes();
            if(!size){
                        ...
                        if(api.mode!=='visual'){
                            const check=styling=>{
                                ...
                                for(let i=paddingLen-1;i>-1;--i){
                                    if(styling[colPaddingsIds[i]+'_unit']==='%' && styling[colPaddingsIds[i]]!=='' && styling[colPaddingsIds[i]]!==und && !styling[colPaddingsIds[i]].toString().includes(',')){
                                        return true;
                                    }
                                    for (let j = bpLength - 2; j>-1; --j) {
                                        if(styling['breakpoint_'+points[j]]){
                                            let p=styling['breakpoint_'+points[j]][colPaddingsIds[i]];
                                            if(p!=='' && p!==und && !p.toString().includes(',') && ThemifyStyles.getStyleVal(colPaddingsIds[i]+'_unit',styling,points[j])==='%'){
                                                return true;
                                            }
                                        }
                                    }
    ...
    api.Utils = {
        ...
        findCssRule(rules, selector) {
            ...
            for (let i = rules.length - 1; i > -1; --i) {
                if ((isCondition===true && rules[i].conditionText && rules[i].cssText.replace(/\s*>\s*/g, '>').replace(/\,\s/g, ',').includes(selector)) || (isCondition===false && !rules[i].conditionText && selector === rules[i].selectorText.replace(/\s*>\s*/g, '>').replace(/\,\s/g, ','))) {
                    return i;
                } 
            }
            return false;
                            ...
                            if (styling && Object.keys(styling).length > 0) {
                                if(saving===true && api.mode==='visual'){//in admin part we can't use iframe to convert paddings that is why skipping
                                    ...
                                    for(let j=paddingLen-1;j>-1;--j){
                                        ...
                                        if(styling[prop+'_unit']==='%' && styling[prop]!=='' && styling[prop]!==und && !styling[prop].toString().includes(',')){
                                        ...
                                        for (let k = bpLength - 2; k>-1; --k) {
                                            if(styling['breakpoint_'+points[k]]!==und){
                                                ...
                                                if(p!=='' && p!==und && !p.toString().includes(',') && ThemifyStyles.getStyleVal(prop+'_unit',styling,points[k])==='%'){
                                ...
                                if(fields[i]===und){
                                    ...
                                    if(Array.isArray(fields[i]) || typeof fields[i]==='object'){
                                        ...
                                        if(v){
                                            ...
                                            else if(isNaN(v) && (v[0]==='[' || !v.includes(' '))){
                                                for(let [k,img] of res){
                                                    if(img!==false && v.includes(k)){
                    ...
                    checkPaddings=(bpStyles,allStyles,bp)=>{
                        ...
                        for(let i=colPaddingsIds.length-1;i>-1;--i){
                            let prop=colPaddingsIds[i],
                                p=bpStyles[prop];
                            if(p!=='' && p!==und && !p.toString().includes(',') && ThemifyStyles.getStyleVal(prop+'_unit',allStyles,bp)==='%'){
                                found[prop]=p;
                            }
                        }
                        return found;
                                ...
                                for (let elementId in elements[bp]) {
                                    ...
                                    for (let propId in elements[bp][elementId]) {
                                        let value =elements[bp][elementId][propId],
                                            ...
                                            if(bp==='desktop'){
                                                if(styles[propId]!==und && styles[propId].toString().includes(',')){
                                                    continue;
                                                }
                                                ...
                                                if(styles['breakpoint_'+bp]!==und && styles['breakpoint_'+bp][propId]!==und && styles['breakpoint_'+bp][propId].toString().includes(',')){
                                                    continue;
                                                }
                                            ...
                                            if(propId[0]!=='p'){
                                                ...
                                                if(isAllChecked && elements[bp][elementId].padding_top!==und && !isAllChecked.includes(' ')){
    ...
    win.tfOn('offline', () => {
        setTimeout(()=>{
            if(!win.navigator.onLine && !api.Builder.get().isSaved && api.undoManager.hasUndo()){
                ...
                msgWrap.innerHTML=themifyBuilder.i18n.offline;
