...
( ( Themify,doc,vars ) => {
	...
	ThemifyBuilderPage = {
		el : null,
		layoutsList() {
            return Themify.fetch('', null, fetchArgs, vars.paths.predesigned.url);
		},
		publish( formData ) {
        ...
        submit(form){
            ...
            if ( !slug || slug==='blank') {
                ...
                Themify.fetch('', null, fetchArgs, file).then(res=>{
                    ...
                    if ( JSON.stringify( res ).includes( 'global_styles' ) ) {
                        Themify.fetch('', null, fetchArgs, vars.paths.predesigned.single.replace('{SLUG}', slug + '-gs')).then(res => {
                            data['used_gs']=res;
                        })
                        .finally(() => {
                            __callback(data);
            ...
            if (!el.classList.contains('current')) {
                ...
                for (let i = list.length - 1; i > -1; --i) {
                    ...
                    if (!show) {
                        ...
                        if (cat) {
                            show = text === cat || cat.includes(text);
                        }
                    }
