((api,Themify, doc) => {
    ...
    api.preDesignedRows = class {
        constructor(el) {
            ...
            this.el = el;
            if (!allRows) {
                Themify.fetch('', null, fetchArgs, themifyBuilder.paths.rows_index).then(data => {
                    allRows=data;
                    this.init();
                }, e => {
    ...
    api.preDesignedRows.get = async slug=>{
         api.Spinner.showLoader();
            ...
            try {
                const data = await Themify.fetch('', null, fetchArgs, themifyBuilder.paths.row_template.replace('{SLUG}', slug));
                api.Helper.clearElementId(data);
                rows.set(slug,data);
                // Import GS
                if (JSON.stringify(data).includes(api.GS.key)) {
                    try {
                        const res = await Themify.fetch('', null, fetchArgs, themifyBuilder.paths.row_template.replace('{SLUG}', slug + '-gs')),
                            convert = {};
                        for (let i in res) {
