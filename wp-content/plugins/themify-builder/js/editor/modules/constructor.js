    ...
    ThemifyConstructor = {
            ...
            const lb=this.label,
                options={
                    ...
                    blend:{
                        ...
                        darken: lb.darken,
                        lighten: lb.light,
                        'color-dodge': lb.colord,
                        'color-burn': lb.colorb,
                        difference: lb.diff,
                        exclusion: lb.excl,
                        hue: lb.hue,
                        saturation: lb.sat,
                    ...
                    position:{
                        'left-top':lb.left+' '+ lb.top,
                        'left-center': lb.left+' '+ lb.center,
                        'left-bottom':lb.left+' '+ lb.bottom,
                        'right-top': lb.right+' '+ lb.top,
                        'right-center': lb.right+' '+ lb.center,
                        'right-bottom': lb.right+' '+ lb.bottom,
                        'center-top': lb.center+' '+ lb.top,
                        'center-center': lb.center+' '+ lb.center,
                        'center-bottom': lb.center+' '+ lb.bottom
                    },
                    border:{
                        solid: lb.solid,
                        dashed: lb.dashed,
                    ...
                    border_radius:[
                        {
                            id: 'top',
                            label:lb.top+' '+lb.left
                        },
                        {
                            id: 'right',
                            label: lb.top+' '+lb.right
                        },
                        {
                            id: 'left',
                            label: lb.bottom+' '+lb.left
                        },
                        {
                            id: 'bottom',
                            label: lb.bottom+' '+lb.right
                        }
                    ],
                    choose:{
                        yes:lb.y,
        ...
        binding(_this, data, val, context) {
            ...
            if ('video' === type) {
                ...
                logic = binding.empty;
            } else if (binding[val] !== und && (val || type === 'select')) {
        ...
        getStyleVal(id, bp, vals) {
            ...
            if (id!==und && id !== '' && api.activeModel !== null) {
                    ...
                    if (vals['breakpoint_' + bp] !== und && vals['breakpoint_' + bp][id] !== und && vals['breakpoint_' + bp][id] !== '') {
                        ...
                        for (let i = points.indexOf(bp) + 1, len = points.length; i < len; ++i) {
                            if (points[i] !== 'desktop') {
                                ...
                                if (und !== binding_data && und !== binding_data[vals[id]] && und !== binding_data[vals[id]].responsive && und !== binding_data[vals[id]].responsive.disabled && binding_data[vals[id]].responsive.disabled.includes(id)) {
                                    v= und;
                                }
                                else{
                                    v=vals[id];
                ...
                if ((v===und || v==='') && id.endsWith('_unit')&& !id.includes('frame_')) {//because in the very old version, px wasn't saved and we can't detect after removing it was px value or not
                    v= 'px';
                }
                //for columns it can be "value1,value2" where "value1" is value for v5, "value2" is for v7
                else if ( v !== und && v !== ''  && api.activeModel.type === 'column' && v.toString().includes(',') && (id.indexOf('padding') === 0 || id.includes('margin') ) ) {
                    v=v.trim().split(',');
                    if(v[1]!==und && v[1]!==''){
                        v=v[1];
                    }
        ...
        resetStyling(model) {
            ...
            for (let i in values) {
                let key = i.includes('_color')  ? 'color' : (i.includes('_style')  ? 'style' : false),
                        remove = null;
                if (i.indexOf('breakpoint_') === 0 || i === api.GS.key || styleFields[i] !== und || i.includes('_apply_all')) {
                    remove = true;
                }
        ...
        mediaFile: {
                ...
                uploader.tfOn(Themify.click, e=> {
                    ...
                    if (this._frames[type] !== und) {
                        ...
                        file_frame = wp.media.frames.file_frame = wp.media({
                            title: input.dataset.title || self.label['upload_'+type] || self.label.upload_image,
                            library: {
            ...
            render(type, data, self) {
                ...
                upload_btn.href = '#';
                upload_btn.dataset.libraryType = type==='json'?'text/plain,application/json':type;
        ...
        lottie:{
            getOptions(markers,min,max,self){
                const lb=self.label,
                    lottieLb=lb.lottie,
                    frames={"":lb.none};
            ...
            render(data, self) {
                ...
                data.ext='json';
                data.binding={
                    empty:{hide:'tb_lottie_act_w'},
                    not_empty:{show:'tb_lottie_act_w'}
                };
                const f= doc.createDocumentFragment(),
                    browse=doc.createElement('button'),
                    fileWrap=self.file.render(data, self),
                    ...
                    _callback=async (el,isChange,fid)=>{
                        ...
                        const tmpMarkers=json.markers || [],
                            markers=new Map(),
                            row=el.closest('.tb_repeatable_field');
                        ...
                        let frame=fid && markers.has(fid)?markers.get(fid):{},
                            min=frame.min || 0,
                            max=frame.max?frame.max:(json.op?Math.floor(json.op - json.ip):'');
                        max=parseInt(max) || '';
                        if(!row.tfClass('tb_lottie_act_w')[0]){
                            const origVals=self.values,
                                origRepeat=!!self.is_repeat;
                            self.is_repeat=isRepeat;
                            self.values=vals;
                            el.closest('.tb_field').after(self.create([this.getOptions(markers,min,max,self)]));
                            self.is_repeat=origRepeat;
                            self.values=origVals;
                        }
                        const framesWrap=row.tfClass('tb_lottie_frame_id')[0],
                            frameSelect=framesWrap.tfTag('select')[0],
                            sliderWrap=row.tfClass('tb_lottie_range_id')[0].tfClass('tb_slider_wrapper')[0];
                        if(markers.size===0){
                            framesWrap.style.display='none';
                        }
                        if(isChange){
                            const range=sliderWrap.tfClass('tb_lb_option_child')[0],
                                fr=doc.createDocumentFragment();
                            if(!fid){
                            ...
                            sliderWrap.tfClass('tb_slider_output_low')[0].dataset.slider_after=min;
                            sliderWrap.tfClass('tb_slider_output_high')[0].dataset.slider_before=max;
                            sliderWrap.style.setProperty('--tb_slider_after', min);
                            sliderWrap.style.setProperty('--tb_slider_min', min);
                            sliderWrap.style.setProperty('--tb_slider_before', max);
                            sliderWrap.style.setProperty('--tb_slider_max', max);
                            for(let items=sliderWrap.querySelectorAll('[type="range"]'),i=items.length-1;i>-1;--i){
                                items[i].max=max;
                                items[i].min=min;
                                items[i].value=i===0?min:max;
                            }
                            range.value='';
                            Themify.triggerEvent(sliderWrap,'input');
                        }else{
                ...
                browse.type='button';
                browse.textContent=themifyBuilder.i18n.lottie_lib.browse;
                browse.tfOn(Themify.click,async e=>{
                    api.Spinner.showLoader();
                    try{
                        const opt=themifyBuilder.i18n.lottie_lib;
                        opt.all=self.label.all;console.log(opt);
                        await Themify.loadJs(Themify.url+'js/admin/modules/lottie-browse',!!window.TF_LottieBrowse);
                        await TF_LottieBrowse.run(input,opt,themifyBuilder.nonce);
        ...
        checkboxGenerate(type, data) {
            ...
            for (let i = 0; i < len; ++i) {
                ...
                ch.checked = (v !== und && v !== false && v.includes(options[i].name)) || (_default === options[i].name || (is_array === true && _default.includes(options[i].name)));
                if (data.class !== und) {
        ...
        layout: {
            ...
            render(data, self) {
                ...
                for (let i = 0, len = options.length; i < len; ++i) {
                    ...
                    if (data.mode === 'sprite' && !options[i].img.includes('.png')) {
                        sprite = doc.createElement('span');
                        sprite.className = 'tb_sprite';
                        if (options[i].img.includes('http')) {
                        ...
                        sprite.src = options[i].img.includes('http')? options[i].img : Themify.builder_url + 'editor/img/' + options[i].img;
                    }
        ...
        layoutPart: {
            ...
            render(data, self) {
                ...
                d.append(selectWrap, doc.createElement('br'));
				if ( data.add_url ) {
            ...
            async init(btn, input) {
                ...
                preview = images => {
                    const w = 40,
                        h = 40,
                        fr=doc.createDocumentFragment();
                    let prewiew_wrap=input.parentNode.tfClass('tb_shortcode_preview')[0];
        ...
        select: {
            ...
            make_options(data, v, self) {
                ...
                for (let k in options) {
                    ...
                    opt.text = options[k];
                    // Check for responsive disable
                    if (und !== data.binding && und !== data.binding[k] && und !== data.binding[k].responsive && und !== data.binding[k].responsive.disabled && data.binding[k].responsive.disabled.includes(data.id)) {
        ...
        font_select: {
            ...
            loadGoogleFonts(fontFamilies) {
                ...
                const result = {google: [], cf: []},
                        ...
                        loading = ()=>{
                            for (let i = loaded.length - 1; i > -1; --i) {
                                if (!this.loaded_fonts.includes(loaded[i])) {
                ...
                for (let i = fontFamilies.length - 1; i > -1; --i) {
                    if (fontFamilies[i] && !this.loaded_fonts.includes(fontFamilies[i])  && (!result.google.includes(fontFamilies[i]) || !result.cf.includes(fontFamilies[i]))) {
                        ...
                        if (!this.loaded_fonts.includes(f)) {
            ...
            controlChange(select, preview, pw, self) {
                ...
                $combo[0].tfOn(Themify.click, function (e) {
                    ...
                    if (target.classList.contains('themify-combo-item')) {
                        ...
                        if ('webfont' !== type && value && !_this.loaded_fonts.includes(value)) {
        ...
        animation_select: {
            render(data, self) {
                const select_wrap = doc.createElement('div'),
                        select = doc.createElement('select'),
                        options = self.getOptions('preset_animation'),
                        v = self.values[data.id];
                select_wrap.className = 'selectwrapper tf_inline_b tf_vmiddle tf_rel';
        ...
        position: {
            render(data, self) {
                ...
                if(self.component!=='column'){
                    ...
                    select.binding.relative={show: 'tb_group_element_position'};
        ...
        transform: {
            ...
            render(data, self) {
                ...
                // Scale
                ul.appendChild(this._xy('scale', {'': {min: -100, max: 1000, increment:.1}}, data, self, self.label.sc));
                // Translate
