((api, Themify, topWindow, doc,und) => {
    ...
    class GS {
        ...
        events(){
            ...
            this.el.tfClass('actions')[0].tfOn(click,e=>{
                e.stopPropagation();
            ...
            this.selectedContainer.tfOn(click,e=>{
            ...
            this.el.querySelector('#search').tfOn('input', e=> {
            ...
            this.el.tfClass('clear_search')[0].tfOn(click, e=> {
                e.stopPropagation();
            ...
            this.el.parentNode.querySelector('.overlay').tfOn(click, function(e) {
                e.preventDefault();
                e.stopPropagation();
                this.remove();
            },{once: true});
        ...
        async saveAs() {
            const data=await api.LiteLightBox.prompt(themifyBuilder.i18n.enterGlobalStyleName);
            if(data[0]==='yes'){
                const title=data[1];
                if (!title) {
                    ...
                    try{
                        ...
                        if ('success' === res.status) {
                            ...
                            res = res.post_data;
                            await api.Utils.saveCss(api.Helper.clear(res.data), '', res.id);
        ...
        destroy(){
            ...
            this.field=this.list=xhr =this.el= this.selectedContainer=this.done=this.reLoad=null;
        }
    }
                        ...
                        try{
                            api.Utils.saveCss(gsPost.data, '', gsPost.id).then(()=>{
                                ...
                                if (api.mode === 'visual') {
                                    ...
                                    for (let [_cid,model] of items) {
                                        let args = model.get('styling');
                                        if (args[this.key] && args[this.key].includes(id)) {
