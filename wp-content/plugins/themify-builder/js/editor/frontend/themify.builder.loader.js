...
((Themify, win, doc) => {
        ...
        Themify.fetch({action:'tb_load_editor'}, 'text').then(res => {
            ...
            for (let i = 0, len = items.length; i < len; ++i) {
                ...
                if (items[0].tagName !== 'SCRIPT' || (items[0].type && items[0].type !== 'text/javascript')) {
                    ...
                    if (!src) {
                        let id = items[0].id;
                        if (!id || !id.includes('themify-main-script')) {
                            let html = items[0].innerHTML;
                            s.innerHTML = html;
                            if (!html.includes('tinyMCEPreInit.')&& !html.includes('.addI18n') && !html.includes('.i18n') && !html.includes('wp.editor')) {
                                fr.appendChild(s);
                            } else {
                                final.push(s);
                            }
                        }
                    } else if (needToLoad[src] === undefined && !src.includes('themify/js/main')&& doc.querySelector('script[src="' + src + '"]') === null) {
                        s.async = false;
                        needToLoad[src] = true;
    ...
    windowLoad = () => {
        let pageId,
            responsiveSrc = win.location.href.includes('?') ? '&' : '?';
        $ = jQuery;
                ...
                if (tbLoaderVars.styles !== null) {
                    for (let i  in tbLoaderVars.styles) {
                        if (tbLoaderVars.styles[i] !== '') {
                            let cssId=i.includes('base.min.css')?'tf_base-css':(i.includes('lightbox.css')?'themify-builder-lightbox-css':null),
                                ...
                                for (let i = css.length - 1; i > -1; --i) {
                                    if (css[i] && css[i].parentNode) {
                                        let href = css[i].href,
                                                id = css[i].id;
                                        if (href) {
                                            if (!cssItems[href] && !href.includes('themify.builder.loader') && !href.includes('wp-includes') && !href.includes('admin-bar') && !href.includes('themify-builder/css/editor/modules')) {
                                                css[i].remove();
                                            }
		...
		if (!doc.body.classList.contains('tb_restriction') && !win.location.href.includes('tb-id')) {
