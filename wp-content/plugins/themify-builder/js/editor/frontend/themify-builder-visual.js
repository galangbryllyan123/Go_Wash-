((Themify, win, topWindow, doc, api,und) => {
                ...
                if (e.which === 1) {
                    ...
                    _move = e => {
                        ...
                        timer = requestAnimationFrame(() => {
                            ...
                            tooltip.textContent = iframe.style.width = parseInt(w) + 'px';
                    ...
                    _up= function (e) {
                        ...
                        this.tfOff('lostpointercapture pointerup', _up, {once: true, passive: true})
                            ...
                            if (data.success) {
                                ...
                                if (styles) {
                                    for (let i = 0, len = styles.length; i < len; ++i) {
                                        Themify.loadCss(styles[i].s+'?ver='+styles[i].v, null, false, null, styles[i].m);
                                    }
                                }
    ...
    ThemifyLiveStyling.prototype.setLiveStyle = function (prop, val, selectors) {
        ...
        for (let i = 0, len = selectors.length; i < len; ++i) {
            ...
            if (isPseudo === false && !selectors[i].includes(':hover')) {
                selectors[i] += ':hover';
            }
        ...
        if (this.isChanged === true) {
            ...
            if (this.styleTabId === 'h') {
                ...
                for (let i = hover_selectors.length - 1; i > -1; --i) {
                    if (!hover_selectors[i].includes('tb_visual_hover')) {
        ...
        if (index === false || !rules[index]) {
            ...
            const priority = val !== '' && val.includes('!important') ? 'important' : '';
            if (priority !== '') {
    ...
    ThemifyLiveStyling.prototype.addOrRemoveFrame = function (_this, settings) {
        ...
        if (settings.type === side + '-presets' || settings.type === side + '-custom') {
			...
			/* the bottom and right preset frames are rotated around automatically */
			this.setLiveStyle( 'transform', settings.type === side + '-presets' && ( side === 'bottom' || side === 'right' ) ? 'rotate(180deg)' : '', selector );
        }
        if (!isLive) {
    ...
    ThemifyLiveStyling.prototype.addOrRemoveComponentOverlay = function (type, id, v) {
        ...
        if (v === '' && id) {
            ...
            if (!overlayElmt) {
                ...
                ( this.model.type === 'row' ? this.el.querySelector(':scope > .row_inner') : this.el.tfClass('tb_' + this.model.type + '_action')[0] ).before(overlayElmt);
            }
    ...
    ThemifyLiveStyling.prototype.bindBackgroundMode = function (val, id) {
        ...
        if (bgValues[val] !== und) {
            ...
            item = suffix!=='' || (type && type !== 'image') || getInputValue('resp_no_bg', '#resp_no_bg input:checked') ? null :(args?getInputValue(args.origId, '#' + args.origId):null);

            if (item && item === '') {
                val = null;
            } else if(args){
                const posId = 'background_position'+suffix,
                ...
                if(api.activeBreakPoint!=='desktop'){
                    if(propCSS['background-attachment']===''){
                        propCSS['background-attachment']=getInputValue('background_attachment'+suffix, '#background_attachment'+suffix) || '';
                    }
                    if(propCSS['background-position']===''){
                        propCSS['background-position']=getInputValue(posId, '#'+posId) || '';
                    }
                }
            }
            else{
    ...
    ThemifyLiveStyling.prototype.bindFontColorType = function (v, id, type) {
        if (type === 'radio') {
            const is_color = v.includes('_solid'),
        ...
        el.tfOn(event, function (e) {
            ...
            if (is_select === true) {
                ...
                if (this.closest('#background_video_options') !== null) {
                    ...
                    return;
                } else if (('height' === prop && id.includes('_auto_height')) || ('width' === prop && id.includes('_auto_width'))) {
    ...
    ThemifyLiveStyling.prototype.removeBgVideo = function () {
        ...
        const video = this.el.querySelector(':scope>.tb_fullwidth_video');
        if (video !== null) {
            video.remove();
        }
    };
    ...
    api.EdgeDrag = {
        ...
        getCurrentStyling(id, model, type) {
            ...
            if (v !== und && v !== '' && model.type === 'column' && v.toString().includes(',')  && (id.indexOf('padding') === 0 || id.includes('margin'))) {
                v = v.trim().split(',');
                if (v[1] !== und && v[1] !== '') {
                    v = v[1];
                } else {
        ...
        drag(e) {
            if (e.which === 1) {
                ...
                if (model) {
                    ...
                    el.tfOn('lostpointercapture pointerup', _up, {once: true, passive: true})
