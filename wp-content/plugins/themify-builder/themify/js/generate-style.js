...
(function (win, doc,und) {
    ...
    ThemifyStyles = {
        ...
        init(data, breakpointsReverse, bid, gutters) {
        ...
        getArea(areaValue, cssValue, bp, count) {
            var res = null,
                responsiveKey,
                globalKey;
        ...
        generateGSstyles(gsItems, elType, gsClass) {
        ...
        createCss(data, elType, saving, gsClass, isGSCall, optimize) {
            if (!elType) {
                elType = 'row';
            }
            this.saving = saving;
        ...
        getFieldCss(elementId, module, settings, allData) {
        ...
        fields: {
            frameCache: {},
            imageGradient(id, type, args, data, elementId, allData) {
                var selector = false,
            ...
            image(id, type, args, data, elementId, allData) {
            ...
            gradient(id, type, args, data, elementId, allData) {
                var selector = false,
                        origId = args.id,
            ...
            icon_radio(id, type, args, data, elementId, allData) {
                var v = this.getStyleVal(id, allData);
                return !v ? false : args.prop + ':' + v + '#@#';
            },
            color(id, type, args, data, elementId, allData) {
            ...
            fontColor(id, type, args, data, elementId, allData) {
                var v = this.getStyleVal(id, allData),
                        selector = false;
            ...
            gap(id, type, args, data, elementId, allData) {
                var v = this.getStyleVal(id, allData);
                return v===und? false : args.prop + ':' + v + '#@#';
            },
            padding(id, type, args, data, elementId, allData) {
                var prop = args.prop,
                ...
                if(unit==='%' && type==='column' && split[0] && split[1]===und && ['padding_top','padding_bottom','padding_left','padding_right','margin-bottom','margin-top'].indexOf(id)!==-1){
            ...
            box_shadow(id, type, args, data, elementId, allData) {
                var prop = args.prop,
                        origId = args.id,
                        v = this.getStyleVal(id, allData);
                if (v === und || v === '') {
            ...
            border_radius(id, type, args, data, elementId, allData) {
                var origId = args.id,
            ...
            border(id, type, args, data, elementId, allData) {
                var prop = args.prop,
                        origId = args.id,
                        val,
                        v = this.getStyleVal(id, allData);
            ...
            select(id, type, args, data, elementId, allData) {
                var prop = args.prop,
                        selector = '',
                        v = this.getStyleVal(id, allData),
                        suffix=id.endsWith('_inner')?'_inner':'';

                if (v === und || v === '' || prop === 'column-rule-style') {
                    return false;
                }
                if (prop === 'background-mode' || prop === 'background-repeat' || prop === 'background-attachment') {
                    if (!data[args.origId] || (!suffix && this.getStyleVal('resp_no_bg', allData))) {
                        return false;
                    }
                    var selectedType=suffix?null:this.getStyleVal('background_type', allData);
                    ...
                    if (prop === 'background-mode') {
                        ...
                        if (bg_values[v] !== und) {
                            if (v.indexOf('repeat') !== -1) {
                                ...
                                if (v === 'best-fit-image' || v === 'builder-parallax-scrolling' || v === 'builder-zoom-scrolling' || v === 'builder-zooming') {
                                    ...
                                    var pos = this.fields.position_box.call(this, 'background_position'+suffix, type, tmp, data, elementId, allData) ||  'background-position:center#@#';
                                    tmp = null;
                                    selector += pos;
                        ...
                        if(!suffix && origV !== 'builder-parallax-scrolling' && origV !== 'builder-zooming' && origV !== 'builder-zoom-scrolling'){
                            selector += '--tbBg:0#@#';
                        }
                    }
                        ...
                        var bgMode=  this.getStyleVal('background_repeat'+suffix, allData);
            ...
            position_box(id, type, args, data, elementId, allData) {
                var prop = args.prop,
                        v = this.getStyleVal(id, allData),
                        bp = '';
            ...
            font_select(id, type, args, data, elementId, allData) {
            ...
            frame(id, type, args, data, elementId, allData) {
                return false;
            },
            range(id, type, args, data, elementId, allData) {
            ...
            radio(id, type, args, data, elementId, allData) {
                if (args.prop === 'frame-custom') {
					...
					if ( v === side + '-presets' && ( side === 'bottom' || side === 'right' ) ) {
						selector += 'transform: rotate(180deg)#@#';
					}
                    var shadow = [
            ...
            multiColumns(id, type, args, data, elementId, allData) {
            ...
            height(id, type, args, data, elementId, allData) {
                var prop = 'height',
                        selector = false;
            ...
            text(id, type, args, data, elementId, allData) {
                var v = this.getStyleVal(id, allData),
                        selector = false;
                if (v !== und && v !== '') {
                    selector = args.prop + ':' + v + '#@#';
            ...
            number(id, type, args, data, elementId, allData) {
                return  this.fields.text.call(this, id, type, args, data, elementId, allData);
            },
            width(id, type, args, data, elementId, allData) {
                var prop = args.prop,
                        v,
                        selector = false,
                        v = this.getStyleVal(id, allData);
            ...
            position(id, type, args, data, elementId, allData) {
                var selector = false,
                        v = this.getStyleVal(id, allData);
                if ('' !== v) {
                    selector = 'position:' + v + '#@#';
                    if ( 'absolute' === v || 'fixed' === v || 'relative' === v ) {
            ...
            transform(id, type, args, data, elementId, allData) {
                var selector = '',
                        v, x, y, unit,
            ...
            grid(id, type, args, data, element_id, allData, bp, as_array) {
                if (!bp) {
                    bp = this.breakpoint;
                }
        ...
        gridBackwardCompatibility(cols) {//backward compatibility convert old cols to new data
