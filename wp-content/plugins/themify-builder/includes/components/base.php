...
class Themify_Builder_Component_Base {
	...
	elseif (Themify_Builder::$frontedit_active === false && (!empty($fields_args['motion_effects']) || !empty($fields_args['custom_parallax_scroll_speed']) )) {
		...
		// Vertical
		if (isset($fields_args['motion_effects']['v']['val']['v_dir']) && $fields_args['motion_effects']['v']['val']['v_dir'] !== '') {
		    $has_lax = true;
		...
		// Horizontal
		if (isset( $fields_args['motion_effects']['h']['val']['h_dir']) && $fields_args['motion_effects']['h']['val']['h_dir'] !== '') {
		    $has_lax = true;
		...
		// Opacity
		if (isset( $fields_args['motion_effects']['t']['val']['t_dir']) && $fields_args['motion_effects']['t']['val']['t_dir'] !== '') {
		    $has_lax = true;
		...
		// Blur
		if (isset($fields_args['motion_effects']['b']['val']['b_dir']) && $fields_args['motion_effects']['b']['val']['b_dir'] !== '') {
		    $has_lax = true;
		...
		// Rotate
		if (isset( $fields_args['motion_effects']['r']['val']['r_dir']) && $fields_args['motion_effects']['r']['val']['r_dir'] !== '') {
		    $has_lax = true;
		...
		// Scale
		if (isset($fields_args['motion_effects']['s']['val']['s_dir']) && $fields_args['motion_effects']['s']['val']['s_dir'] !== '') {
		    $has_lax = true;
	...
	foreach ($sides as $side) {
		...
		foreach ($breakpoints as $bp => $v) {
		    ...
		    if (!empty($settings) && ( $frame = Themify_Builder_Model::get_frame( $settings, $side ) ) ) {
				$printed[$side] = true;
