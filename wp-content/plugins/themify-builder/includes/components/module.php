...
class Themify_Builder_Component_Module extends Themify_Builder_Component_Base {
    ...
    public function print_template($echo=false,$ignoreLocal=false) {
        ob_start();
        $template_file = self::locate_template( 'template-' . $this->slug . '-visual.php' );
        if($ignoreLocal===false && is_file( $template_file ) ) {
        ...
        $output =$ignoreLocal===false? '<script type="text/template" id="tmpl-builder-'.$this->slug.'">'.$content_template.'</script>':$content_template;
        if($echo===false){
            return $output;
        }
        echo $output;
	...
	public function get_form_settings( $tab = '' ) {
		if($tab===true){
		    $tab='styling';
		}elseif($tab===false){
		    $tab='';
		...
		if($tab==='styling' || $tab===''){
		    ...
		    $styles = self::add_zindex_filed($styles);
		    $styles = self::add_transform_filed($styles);
		    if($tab==='styling'){
			return $styles;
		    }
		}
		if($tab==='setting' || $tab===''){
		    $setting =  apply_filters( 'themify_builder_module_settings_fields', $this->get_options(), $this );
		    if($tab==='setting'){
			return $setting;
		    }
		}
		...
		if ( !empty( $styles ) ) {
			$module_form_settings['styling'] = array( 'options' => $styles );
		}
