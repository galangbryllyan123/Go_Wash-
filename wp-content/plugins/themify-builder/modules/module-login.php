...
class TB_Login_Module extends Themify_Builder_Component_Module {
	...
	return array(
		...
		'after' => __( 'You can use: %username%, %display_name%, %first_name%, %last_name%, %email%', 'themify' ),
	    ),
		array(
			'id' => 'logout_link',
			'type' => 'toggle_switch',
			'label' => __('Logout Link', 'themify'),
			'options' => array(
				'on'=>array('name'=>'show','value' =>'s'),
				'off'=>array('name'=>'hide','value' =>'hi')
			),
			'binding' => array(
				'hide' => array( 'hide' => 'logout_redirect' ),
				'show' => array( 'show' => 'logout_redirect' ),
			)
		),
		array(
			'id' => 'logout_redirect',
			'type' => 'url',
			'label' => __('Redirect URL after logout', 'themify'),
			'control'=>false
		),
	    array(
            'id' => 'alignment',
