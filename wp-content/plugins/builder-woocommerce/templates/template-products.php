...
if (themify_is_woocommerce_active()):
	...
	$query = new WP_Query(apply_filters("themify_builder_module_{$mod_name}_query_args", $query_args, $fields_args));

	if ($fields_args['hide_empty'] === 'yes' && !$query->have_posts() && Themify_Builder::$frontedit_active === false) {
		return;
	}

	$container_class = apply_filters('themify_builder_module_classes', array(
		'module', 'woocommerce', 'module-' . $mod_name, $element_id, $fields_args['css_products']
		...
		if ($query->have_posts()) {
			...
			if ($isSlider === true) {
				...
				$container_inner = array(
					...
					'data-speed' => $fields_args['speed_opt_slider'],
					...
					'data-css_url' => Builder_Woocommerce::$url . 'assets/modules/slider'
				);
			...
			$container_inner['class'] = apply_filters('themify_loops_wrapper_class', $class, 'product', $layout, 'builder', $fields_args, $mod_name);
			...
			if (isset($themify)) {
                ...
                /* Shoppe and Ultra */
                if ( isset( $themify->products_hover_image ) ) {
                    $themify->products_hover_image = $fields_args['hover_image'] === 'yes' ? ( $themify->products_hover_image === 'enable' ? 'enable' : 'first_image' ) : false;
				}
			}
			if ($fields_args['post_filter'] === 'yes') {
				...
				unset($filter_args);
			} else if ( $fields_args['sort'] === 'yes' ) {
                wc_set_loop_prop( 'total', $query->found_posts );
                woocommerce_catalog_ordering();
            }
			TB_Products_Module::set_filters($fields_args);
			$container_inner = apply_filters('themify_builder_blog_container_props', $container_inner, 'product', $layout, $fields_args, $mod_name);
			if ( in_array( $fields_args['hide_page_nav_products'], [ 'loadmore', 'infinite' ], true ) || true === $is_ajax_filter ) {
				if ( $fields_args['hide_page_nav_products'] !== 'infinite' ) {
                    $container_inner['class'][] = 'tb_ajax_pagination';
                }
				$container_inner['data-id'] = $element_id;
			}
			if ($fields_args['post_filter'] === 'yes') {
