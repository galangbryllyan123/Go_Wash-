...
class TB_Products_Module extends Themify_Builder_Component_Module {
	...
	public static function set_filters(array $args){
        ...
        if ( $args['hide_sales_badge'] === 'yes' ) {
            add_filter( 'wc_get_template', [ __CLASS__, 'hide_sales_badge' ], 10, 2 );
        }
	}

    public static function hide_sales_badge( $template, $template_name ) {
        if ( $template_name === 'loop/sale-flash.php' ) {
            $template = dirname( __DIR__ ) . '/templates/partials/blank.php';
        }
	...
	public static function revert_filters(){
		if(self::$isLoop===true){
            ...
            remove_filter( 'wc_get_template', [ __CLASS__, 'hide_sales_badge' ], 10 );
