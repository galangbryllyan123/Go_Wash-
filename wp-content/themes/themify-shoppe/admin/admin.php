...
function themify_theme_setup_metaboxes($meta_boxes=array(), $post_type='all') {
    $supportedTypes=array('post', 'page','product', 'portfolio');
    ...
    */
    function themify_check_ecommerce_environment_admin() {
	    ...
	    if ( ! get_option( 'themify_warning_' . $warning ) ) {
		     wp_enqueue_script( 'themify-admin-warning', THEME_URI . '/admin/js/themify.admin.warning.js', array('jquery'), Themify_Enqueue_Assets::$themeVersion, true );
...
function themify_admin_script_style() {
    wp_enqueue_script('themify-admin-script', THEME_URI . '/admin/js/admin-script.js',null,Themify_Enqueue_Assets::$themeVersion,true);
}
